...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-EOF
     ...
     sudo /opt/puppetlabs/puppet/bin/puppet apply -e 'host { "${fqdn}": ensure => present, host_aliases => ["puppet", "puppetdb"], ip => "${ipaddress}", }' --modulepath /tmp
